...
- 🔍 **多引擎搜索** —— Google / 百度 / Bing / DuckDuckGo / 站内书签 / 自定义引擎，下拉切换，回车搜索，`/` 快捷聚焦；聚焦即弹**搜索历史**，输入时模糊匹配、单条可删；中文输入法选字期间的回车只用于上屏，不会误提交。
...
- 按 `/` 快速聚焦搜索框；聚焦后会弹出最近搜索记录，方向键选择、回车直接搜；鼠标移到某条上点右侧 ✕ 可删除该记录。
